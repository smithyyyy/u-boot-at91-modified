...
#ifndef CFG_NO_FLASH
#define CFG_NOR_ENV_ADDR	(PHYS_FLASH_1 + 0xe000)
#define CFG_NOR_ENV_SIZE	0x2000
...
void flash_identification (flash_info_t * info)
{
	...
	case ATM_MANUFACT :
	  ...
	  } else if (device_code == (ATM_ID_BV322D & FLASH_TYPEMASK)) {
	    info->flash_id |= ATM_ID_BV322D & FLASH_TYPEMASK;
	    device_name = "AT49BV322D (32Mbit)";
	  }
	...
	case AMD_MANUFACT :
		...
		else if(device_code == (AMD_ID_GL032N & FLASH_TYPEMASK)){
			info->flash_id |= AMD_ID_GL032N & FLASH_TYPEMASK;
			device_name = "S29GL032N (32Mbit)";
		}
	break;
...
ulong flash_init (void)
{
	...
	for (i = 0; i < CFG_MAX_FLASH_BANKS; i++) {
		...
		if ((flash_info[i].flash_id & FLASH_TYPEMASK) ==
			...
			flash_nb_blocks = sizeof (OrgAT49BV322D) / sizeof (OrgDef);
		} else if ((flash_info[i].flash_id & FLASH_TYPEMASK) ==
			...
			flash_nb_blocks = sizeof (OrgS29JL032H) / sizeof (OrgDef);
                        ...
                        (AMD_ID_GL032N & FLASH_TYPEMASK)){      /* S29GL032N Flash */

                        pOrgDef = OrgS29GL032N;
                        flash_nb_blocks = sizeof (OrgS29GL032N) / sizeof (OrgDef);
                }
		 else {
			flash_nb_blocks = 0;
			pOrgDef = OrgAT49BV16x4;
	...
	flash_protect (FLAG_PROTECT_SET,
		       CFG_NOR_ENV_ADDR,
		       CFG_NOR_ENV_ADDR + CFG_ENV_SIZE - 1, &flash_info[0]);
...
void flash_print_info (flash_info_t * info)
{
	...
	case (ATM_ID_BV6416 & FLASH_TYPEMASK):
		...
		break;
	case (AMD_ID_GL032N & FLASH_TYPEMASK):
		printf ("S29GL032N (32Mbit)\n");
		break;
	default:
		printf ("Unknown Chip Type %x %x\n", info->flash_id,
					info->flash_id & FLASH_TYPEMASK);
		goto Done;
		break;
	}
